...
# --- CRITICAL ASSUMPTION ---
# This script assumes 'p_bp', 'p_cc', and 'p_mf' are already created ggplot objects 
# and that they include their desired titles and aesthetics (Color/Size mapping).

# --- 1. SET UP THE PLOT STRUCTURE ---

# The patchwork syntax '/' means stack plots vertically.
# The patchwork syntax '+' means place plots side-by-side horizontally.

# The layout: Plot A on top (full width), Plot B and C on the bottom (half width each).
# By default, patchwork maintains the legends within each subplot.
combined_plots <- p_bp / (p_cc + p_mf)

# --- 2. APPLY GLOBAL STYLING ---

# Use the 'plot_layout' to ensure A takes up as much space as B+C combined.
# The 'guides = "collect"' argument is commented out, as we are intentionally NOT collecting them.
final_plot <- combined_plots + 
    plot_layout(
        ncol = 1,
        # Give Plot A roughly the same height as the B+C row combined
        heights = c(1.2, 1) 
    ) +
    plot_annotation(
        title = "Combined Gene Ontology Enrichment Analysis (Divided Layout)",
        theme = theme(
            # Style the main figure title
            plot.title = element_text(face = "bold", hjust = 0.5, size = 18)
        )
    ) & # The '&' operator applies the following theme elements to ALL subplots
    theme(
...
png("plot_4_Combined_GO_Analysis.png", 
    width = 16, 
    height = 16, 
    units = "in", # Set the units to inches
    res = 300)    # Set resolution to 300 DPI for high quality

print(final_plot)
dev.off()
cat("Saved plot_4_Combined_GO_Analysis.pnf (All three plots combined).\n")
